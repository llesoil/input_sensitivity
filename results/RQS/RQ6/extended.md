...
### Q-D. Does the experimental protocol include several hardware platforms?

### Q-E. Does the paper train a performance model on compile-time options?

### Q-F. Does the paper train a performance model on run-time options?

### Q-G. Does the paper propose a solution to generalize the performance model across environments? 

Finally, we check if the paper proposes a solution solving deep variability i.e., if they are able to train a performance model that is general and robust enough to predict a near-optimal configuration for any environment. 

Feel free to contact us if you disagree with the results, or if any comment seems unfair to you!

## RESULTS

The rest of this document details the results for each paper and justifies the answers, from Q-A to Q-G. 
...
Q-D:
No
"All experiments have been performed on the same Windows 8 machine with
Intel Core i7-5600U CPU 2.60 GHZ and 8GB RAM."

Q-E:
No, only run-time parameters.
...
Q-D:
Yes, "changes in the environment, changes in the workload, changes in the dataset, and changes in the hardware"

Q-E:
No

Q-F:
Yes, "use this model to reason about its performance at runtime"
...
Q-D:
Yes, "measured on three hardware system"
...
(AM note: they sometimes mix inputs' changes with versions' changes and hardwares' changes)
...
Q-D:
No difference of measurements under different hardware platforms, although it is measured with various computers. "Hence, even though it required over 60 days of measurement with multiple computer". Since there is no separation between the different datasets, No.
...
Q-C : 
No mention of input sensitivity, No
Mention of hardware sensitivity "For example, the SaC compiler has a default configuration that may have suboptimal performance for some hardware platforms."

Q-D:
Apparently all is measured on the same server responsible of the learning. "The learning procedure was conducted on a dedicated server with an Intel Xeon E5-2609, 2.5 GHz and 128 GB RAM, running Ubuntu 14.04" so No unless we missed it.

Q-E:
Apparently only run-time options even if it is mentioned that systems accept both parameters at compile- and load-time. No.
...
Yes for Apache Storm, it considers several inputs but coming from another paper
...
Q-D:
Yes, on the 21 environments. "The experiments were conducted with all the above mentioned benchmarks on 5 cloud clusters."
...
Q-D:
Yes, at least clusters C1 C2 and C3.

Q-E:
No, since llvm is used as a system, the compliation is the run-time varibility of the compiler. 

Q-F:
Yes, cf the configuration options column in Table 1.
...
Q-D:
No. "All virtual environments in these measurements use similar hardware conﬁguration."
...
Q-A : 
No, the video generator do not consider any input data, it is generating videos.
...
Q-F:
Yes, the configurations are given at run time.
...
Q-D:
No, mention of a hardware change, but the video generator has been processed on one hardware plaform.

Q-E:
No according to the datasets.

Q-F:
Yes

Q-G : 
The context features could be used to learn the context of the executing environment so yes, definitely. 
...
Q-D:
Yes, definitely the purpose of the paper.

Q-E:
No, not in the datasets.

Q-F:
Yes, at run time.

Q-G : 
Yes, if we consider that transfer learning can be applied across distinct environments.
...
Q-D:
No, see "All measurements were run on a 64-bit Windows 7 workstation, equipped with an Intel i7-2600 CPU (2 × 3.7 GHz) and 8 GB of RAM, with 2 GB being assigned to the simulator."

Q-E:
Not specified.

Q-F:
The run-time variability is the center of interest of the paper.

Q-G : 
Not really possible to predict performance accoridng to different environments, simply not the main contribution of the paper. 
...
Q-D:
Not specified, no mention of hardware or server in the experimental setup. 

Q-E:
Yes, it is the compile-time variability of LaTeX.

Q-F:
No run-time varibility.
...
Q-D:
No, same as SPLConqueror dataset

Q-E:
No (llvm considered but it is the run-time variability of a compiler).
...
Q-E:
No known compile-time parameter.

Q-F:
Yes

Q-G : 
No, although adversarial learning could improve our performance prediction of the different environments, it is not a possible solution for solving deep variability.
...
Q-D:
No, no mention of other hardware platfroms

Q-E:
Yes, SQLITE_TEMP_STORE is a compile-time parameter of SQLite.

Q-F:
Yes, StandardSize or PageSizeLimit are run-tie parameters.
...
Q-D:
Only a hardware platform per configurable system, so no explicit variations of configuration options ^ hardware platforms.

Q-E:
No, seems to be exclusively run-time options.

Q-F:
Yes.

Q-G : 
No, not really the point, except if we consider the feature interactions on the system level. 
...
Q-D:
Using the SPLConqueror benchmark, no
also for the rest of the experiment, no 
...
Q-D:
No variations, cf "The benchmarks for both systems were conducted on an Intel i5-4590, 16 GB RAM, 256 GB SSD, Ubuntu 16.04."

Q-E:
Yes, "A user can configure the operation of
these modules by enabling or disabling their features through
compile-time options. For example, Core can be configured
to operate safely in a multithreaded environment by enabling
the SQLITE THREADSAFE feature."
...
Q-D:
No, cf "All studies were conducted [...] " in section 5
...
Q-D:
the main variation point of the paper remains the used dbms, and not the hardware.
teh evaluation is made in one hardware accroding to this sentence : "We will show in subsequent sections
that the metrics available to OtterTune are sufficient to distinguish
between workloads for DBMSs deployed on the same hardware."
No.
...
So, for the second question of this paper, we choose to answer No, even if the protocol is well-designed and really impressive in terms of measurement efforts.

Q-C : 
No

Q-D:
No, one  hardware version per system, cf above.
...
Q-D:
"we varied hardware on which the network is deployed (easy) and varied hardware together with the specific deep learning framework (hard), see Table 1 for details."
Yes
...
Q-D:
No, cf "The time is measured running BO4CO on a MacBook Pro with 2.5 GHz Intel Core i7 CPU and 16GB of Memory"
...
Q-D:
No, cf "We performed the measurement runs on a desktop PC with an Intel Core 2 Duo 3.0 GHz
processor, 6 GB RAM and Windows 7 as operating system installed."
...
Q-D:
Yes, cf "The design space consists of 206 different hardware implementations of a sorting network for 256 inputs."
...
Q-E:
No compile-time parameter.

Q-F:
Yes, the parameter related to the 3d printing.
...
Q-D:
Yes, "We measured the running time of TeraSort-30GB on 22 different cloud configurations on AWS EC2 five times."

Q-E:
No mention of compilation

Q-F:
Yes, run-time options of databases
...
Q-D:
No, cf "The experiment was run on a computer with AMD X2 Dual-Core Processor M320, 2.1 GHz clock speed, 4 GB RAM, and Windows 7 as the operating system."

Q-E:
No mention of compile-time options.
...
Q-D:
No, "The algorithms were implemented in Java 7 and the experiments
were carried out on a single Ubuntu 14.04 machine with Intel Core
i7 CPU 2.2 GHz and 8 GB of RAM."

Q-E:
No mention of compile-time options

Q-F:
Yes, the run-time features of table 1.
...
Q-D:
No, cf "Hardware. We run PLEDGE on a laptop with CoreI7-8750H processor and 16GB RAM. The training and testing of neural networks was performed on a Tesla V100-SXM2-16GB GPU."
...
Q-D:
No, cf "We measured all of these variants on a MacBook Pro with a Core i5 2.7GHz and 8GB
RAM, running OS X 10.10 (Yosemite)."
...
Q-D:
Yes, "We use 4 Aliyun ECS instances"
...
Q-D:
No, cf subsection 6.1.1, "– CPU: Intel i3–3240 with 3.4GHz, – GPU: low proﬁle Sapphire Radeon HD7750, – FPGA: Xilinx Spartan 6 (LogiPi board 10 )."

Q-E:
No.

Q-F:
Yes, the image processing operations.
...
Q-D:
No, cf "Tests were conducted on a 32-core (8 ⇥ 4-sockets) Xeon X7550 system running GNU/Linux (Debian 6.0.6)"

Q-E:
Yes, the compilation of programs
...
Q-D:
Yes, same model but different emulated hardware. "That is, a new server is guaranteed 1 CPU, but can be assigned 2 CPUs if the physical machine is not being used by other services (with a 60% probability in our usecase)"

Q-E:
No.

Q-F:
Yes, request at run time
...
Q-D:
No, cf "All of our experiments were run on Linux machines with Intel Xeon X5650 six-core processors, running at 2.66GHz."
...
Q-D:
Yes, see the footnote page 7 : "Intel Core 2 Quad CPU 2.66 GHZ, 4GB RAM, Vista 64Bit; AMD Athlon64 2.2GHz, 2GB RAM, Debian GNU/Linux 7; AMD Athlon64 Dual Core @2.0GHz, 2GB RAM, Debian GNU/Linux 7; Intel Core2 Quad @2.4GHz, 8GB RAM, Debian GNU/Linux 7. Each program was benchmarked on an individual systems."

Q-E:
Yes "Since Berkeley DB C and Java and SQLite use compile-
time conﬁguration"

Q-F:
Yes for the rest of the systems.
...
Q-D:
No, cf "Analyses and measurements were executed on an Ubuntu 18.04 LTS desktop, with a 3.4 GHz 8-core Intel Core i7 processor, 16 GB of RAM, and Java HotSpot TM 64-bit Server VM (v1.8.0_202)."
...
Q-C : 
In a way, Q-A's citation should be enough to say Yes, but not mention to the difference of performance between workloads.
No?

Q-D:
No, cf "The software runs on a standard desktop dual-core machine with 3 GHz per CPU and 3.5 GB RAM."
...
Q-D:
No, cf "Hardware: Analyses and measurements were executed on an Ubuntu 18.04 LTS desktop, with a 3.4 GHz 8-core Intel Core i7 processor, 16 GB of RAM, and Java HotSpot TM 64-bit Server VM (v1.8.0_202)."
...
Q-D:
No, cf "Also, we control external factors like the hardware and workload of the machine by using a grid computing infrastructure called IGRIDA 2 . We take care of using the same hardware and mitigate network-related factors."
...
Q-D:
No, cf "All the experiments in this paper are implemented with Python 3.6, and run on a computer with a 2.2GHz Intel Core i7 CPU, 64GB 1600MHz DDR3 RAM, and a Titan X GPU with 12GB memory, running Red Hat Linux 7.5."
...
Q-D:
No, the ref [18] does not change the hardware
...
Q-G : 
No, for this paper Q-B=No implies Q-D:
...
"The performance of each of the |C|configurations are measured
...
Q-D:
Same cluster and same model of hardware so no, cf "All measurements ran on a cluster of 27 computers, each
...
Q-E:
They compiled the soft manually, but no mention of compile-time option, so no
...
Q-D:
No, cf "All measurements were conducted on clusters of Ubuntu machines with Intel Core 2 Quad CPUs (2.83 GHz) and 8 GB of RAM (xz and lrzip) and 16 GB of RAM (oggenc)."
...
Q-D:
No, cf "We conduct all the experiments automatically on a RedHat Linux system ina High-Performance Computer (HPC) cluster. The basic HPC node is equipped with a 6 core 2.66 GHz Intel Xeon X5650 Westmere, 36 GB of memory, and 256 GB of hard drive."

Q-E:
No

Q-F:
Yes, the configuration options mentioned, both numeric and binary
...
Q-D:
No, cf "We conduct all experiments in a High-Performance Computer (HPC) cluster." 
...
Q-F:
Yes, kernel run-time parameters from benchmarks, see table 1
...
Q-D:
No, cf " we are essentially just repeatedly running an identical software stack, on identical hardware, with identical data inputs, and without contention on our servers"
...
Q-D:
Yes, ten different client machines
...
Q-D:
No, cf "Our study is conducted on a workstation with 16-core Intel(R) Xeon(R) CPU E5-2640 v3, 126G memory, and CentOS 6.10 operating system."
...
Q-D:
Amazon EC2, but no different SKU provided

Q-E:
No

Q-F:
Yes, the adaptable features in table III
...
Q-D:
No, cf "Note that all the time cost here is measured when running all the methods on a Windows 7 computer with Intel Xeon CPU E5-1650 3.2GHz 16GB RAM"
...
Q-D:
No, cf "All our experiments were run on a Linux laptop running Ubuntu 16.04 (one Intel i7-6700HQ 2.60GHz processor with 4 cores, 16GB of memory, and a NVIDIA GTX 1070 GPU)."

Q-E:
No

Q-F:
Yes, (binary features of NN?)
...
It is true it is a border paper, maybe we could remove it.
...
Q-D:
No, cf "In our experiments, we use a Windows 10 computer with Intel Core i7-7600U CPU 2.80GHz with 16GB RAM and Matlab R2017a."
...
Q-D:
Very detailed but unique hardware setup in V-A, so no

Q-E:
No mention of compile-time options 

Q-F:
Yes, the features of MariaDB
...
Q-D:
No, cf "All experiments were conducted on an Ubuntu 16.04LTS machine with 24 Intel Xeon Silver 4116 CPUs @ 2.10GHz and 144GB RAM."

Q-E:
No mention of compile-time options

Q-F:
Yes, the configuration spaces of CBMC, Symbiotic, JayHorn, and JBMC
...
Q-D:
Yes, see table 2, 3 different hardware platforms
